...
Dem_Support <- 
  nc09 %>% 
  count(response == "Dem") %>% 
  filter(`response == "Dem"` == TRUE)

RepCount <- nc09 %>% count(response =="Rep") %>% 
  filter(`response == "Rep"` == TRUE)
UndCount <- nc09 %>% count(response == "Und") %>% 
  filter(`response == "Und"` == TRUE)
...
nc_09 %>% 
  gt() %>% 
  tab_header(title = "North Carolina's Voting Distribution by Race",
             subtitle = "Subtitle") %>% 
  cols_label(
    race_eth = "Ethnicity",
    Dem = "DEM.",
...
Colleagues:
Mike Silva, Adiya Abdilkhay, Seeam Noor, Christopher Onestna, Charlie Flood, Albert Shin
